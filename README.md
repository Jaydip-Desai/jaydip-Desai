<h1 align="center">Hi 👋, I'm Jaydip Desai</h1>
<h3 align="center">Java Developer | Spring Boot Developer | Backend Developer</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Poppins&size=24&duration=3000&color=2E9AFE&center=true&vCenter=true&width=750&lines=Java+Developer;Spring+Boot+Backend+Developer;REST+API+Developer;React.js+%7C+Spring+Boot+%7C+PostgreSQL;AI-Assisted+Development;Always+Learning+New+Technologies" />
</p>

---

# 👨‍💻 About Me

🎓 MCA Graduate

💼 Java Developer with 6 months of hands-on experience in backend application development using Java, Spring Boot, Spring Security, RESTful APIs, Hibernate/JPA, MySQL, and PostgreSQL.

🚀 Experienced in developing and debugging real-world applications, designing REST APIs, and working with database integration.

🤖 Interested in AI-assisted development, AI API integration, microservices, cloud technologies, and modern software development practices.

🌱 Currently Learning
- Microservices Architecture
- System Design
- AWS & Cloud Technologies
- Docker & Containerization
- AI Integration & Generative AI

💡 Interested In
- Java Backend Development
- Enterprise Application Development
- REST API Development
- Microservices Architecture
- AI-powered Applications
- Cloud Technologies

📍 Ahmedabad, Gujarat, India

---

# 🚀 Tech Stack

## 💻 Languages

<p>
<img src="https://skillicons.dev/icons?i=java,js,python"/>
</p>

## ⚙️ Backend

<p>
<img src="https://skillicons.dev/icons?i=spring,maven"/>
</p>

- Spring Boot
- Spring MVC
- Spring Security
- Hibernate / JPA
- REST APIs
- Microservices
- JSP & Servlet

## 🎨 Frontend

<p>
<img src="https://skillicons.dev/icons?i=react,html,css,bootstrap"/>
</p>

- React.js
- HTML5
- CSS3
- Bootstrap

## 🗄️ Database

<p>
<img src="https://skillicons.dev/icons?i=mysql,mongodb,postgresql"/>
</p>

- MySQL
- PostgreSQL
- MongoDB
- Oracle SQL

## ☁️ Cloud & DevOps

<p>
<img src="https://skillicons.dev/icons?i=aws,docker,git,github"/>
</p>

- AWS
- Docker
- Git
- GitHub
- CI/CD

## 🤖 AI & Development

- AI API Integration
- AI-Assisted Coding
- AI-powered Application Development
- Modern AI Development Tools

## 🛠️ Tools

<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,idea,eclipse,postman"/>
</p>

- Postman
- IntelliJ IDEA
- VS Code
- Eclipse
- Maven

---

# 🚀 Featured Projects

## 🤖 AI Resume Analyzer

**Tech Stack**

Java • Spring Boot • React.js • REST APIs • AI API Integration • Maven

### Features

- Resume Upload & Analysis
- Skill and Experience Extraction
- AI-powered Resume Evaluation
- Job-role Alignment Analysis
- Resume Improvement Insights

---

## 🏢 Enterprise Employee CRM (HRMS)

**Tech Stack**

Java • Spring Boot • React.js • MySQL • Spring Security • JWT • Hibernate/JPA

### Features

- Employee Management
- Department Management
- Attendance Tracking
- Leave Management
- Task Management
- Performance Evaluation
- Document Management
- JWT Authentication
- Role-Based Access Control
- Admin Dashboard

---

## 📄 Contract Management System

**Tech Stack**

Java • Spring Boot • PostgreSQL • Hibernate/JPA • REST APIs

### Features

- Contract Lifecycle Management
- Document Management
- Workflow Approval
- Role-Based Authentication
- Invoice Management
- REST APIs
- Notification System
- Secure Backend Architecture

---

## 🍔 Food Delivery System

**Tech Stack**

Java • Spring Boot • React.js • MongoDB • Razorpay

### Features

- Secure User Authentication
- Restaurant & Menu Management
- Shopping Cart
- Razorpay Payment Integration
- Real-time Order Tracking
- Email Notifications
- Admin Dashboard
- Order Management

🔗 Repository  
https://github.com/Jaydip-Desai/Food-Delivery-App

---

## 💎 Jewelry Shop Web Application

**Tech Stack**

JSP • Servlet • MySQL

### Features

- Product Catalog
- Shopping Cart
- Order Placement
- Admin Panel
- MVC Architecture

🔗 Repository  
https://github.com/Jaydip-Desai/E-Jwellary

---

## 🌍 Titanic Foods & Spices

**Tech Stack**

PHP • MySQL

### Features

- Responsive Website
- Product Catalog
- Inquiry Form
- Admin Dashboard

🌐 Live Website  
https://titanicfoodsandspices.com


---

# 🌐 Connect With Me

📧 **Email**  
**djaydip889@gmail.com**

💼 **LinkedIn**  
https://www.linkedin.com/in/desai-jaydip

💻 **GitHub**  
https://github.com/Jaydip-Desai

---

# 💬 Quote

> **"Code with passion, build with purpose, and never stop learning."**

⭐ Thanks for visiting my profile!
